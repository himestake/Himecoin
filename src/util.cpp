...
int nAnonymizeHimecoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Himecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Himecoin
    // Mac: ~/Library/Application Support/Himecoin
    // Unix: ~/.himecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Himecoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "himecoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "himecoind.pid"));
