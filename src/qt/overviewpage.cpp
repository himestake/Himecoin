...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeHimecoinAmount = BitcoinUnits::formatWithUnit(nDisplayUnit, nAnonymizeHimecoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeHimecoinAmount = strAnonymizeHimecoinAmount.remove(strAnonymizeHimecoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHimecoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeHimecoinAmount*COIN) nMaxToAnonymize = nAnonymizeHimecoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeHimecoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeHimecoinAmount));
        strAnonymizeHimecoinAmount = strAnonymizeHimecoinAmount.remove(strAnonymizeHimecoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHimecoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
