...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeHimecoinAmount"))
        settings.setValue("nAnonymizeHimecoinAmount", 1000);
    nAnonymizeHimecoinAmount = settings.value("nAnonymizeHimecoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeHimecoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeHimecoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeHimecoinAmount:
            return QVariant(nAnonymizeHimecoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeHimecoinAmount:
            nAnonymizeHimecoinAmount = value.toInt();
            settings.setValue("nAnonymizeHimecoinAmount", nAnonymizeHimecoinAmount);
            emit AnonymizeHimecoinAmountChanged(nAnonymizeHimecoinAmount);
            break;
        default:
            break;
        }
